...
rownames(featuresTable) = as.character(seq(0,27519))
colnames(featuresTable) = featureList
dim(featuresTable)
write.csv2(featuresTable, file = "featuresTable.csv")

subsetFeatures = function(featTable, nodeList){
  featTableSub = featTable[nodeList+1,]
  return(featTableSub)
}

subFeat8100 = subsetFeatures(featuresTable, unique(nodes8100$id))


```

```{r}
featClean = function(feat){
  for(i in 1:dim(feat)[1]){
    for(j in 1:dim(feat)[2]){
      if(is.na(feat[i,j])){
        feat[i,j] = 0
      }
    }
...
subFeat239 = featClean(subFeat239)
subFeat239K = subFeat239[,c(22,25)]
colnames(subFeat239)

subsetFeatures(featuresTable, nodes239)


subFeat8100 = subsetFeatures(featuresTable, unique(nodes8100$id))
subFeat8100 = featClean(subFeat8100)
subFeat8100K = subFeat8100[,c(13,22,37)]
colnames(subFeat8100K) = c("a", "b", "c")

k1 = casc(adj8100, subFeat8100K, nBlocks = 9)
```
...
# FAST GREEDY #
fastGreedyClust = function(graphK, graphNodesK, centersK){
...
K1 = fastGreedyClust(graph239, nodes239, trueCircles239)

# LEADING EV #
levClust = function(graphK, graphNodesK){
...
clustRes = read_excel("Loss_Result.xlsx")
clustRes = clustRes[-10,]
plot(x = seq(1,9), y = clustRes$FastGreedy, type = "l", col = 1)
points(x = seq(1,9), y = clustRes$FirstEigen, type = "l", col = 2)
points(x = seq(1,9), y = clustRes$Spectral, type = "l", col = 3)
points(x = seq(1,9), y = clustRes$MCL, type = "l", col = 4)
...
fgEst239 = fastGreedyClust(graph239, nodes239, trueCircles239)
fgEst239_2 = fastGreedyClust2(graph239)
...
subFeat239 = as.data.frame(subsetFeatures(featuresTable, nodes239$id))

gendercolor = as.character(subFeat239$`hometown;id`)
gendercolorP = unique(gendercolor)
gendercolorP = gendercolorP[-2]

str_replace(gendercolor, gendercolorP, as.character(seq(1,21)))
...
from <- gendercolorP
to <- as.character(seq(1,21))

gendercolor2 = as.numeric(gsub2(from, to, gendercolor))

gendercolor2 = gsub ("0", "red", gendercolor)#specifying  the  colors for  female  (=1)  and male  (=2)
gendercolor2 = gsub ("1", "blue", gendercolor2)

plot(graph239 , layout = layout.fruchterman.reingold , vertex.color
= gendercolor2 ,
vertex.size = 15, vertex.label.cex = 0.65,  main = "Gender (Red=
F, Blue=M)",
frame = TRUE)

plot(kc, graph239, layout = layout.fruchterman.reingold)
```
...
casc(adjMat, as.matrix(covMat), nBlocks=2)
...
test1 = model.matrix(~ a+b+c -1, data = as.data.frame(subFeat8100K))
